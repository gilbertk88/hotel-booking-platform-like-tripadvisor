<?php if ($apartments): ?>
	<section style="width:100%; padding-bottom:20px">
		<div style="width:960px; margin:0; padding:10px 0 10px 20px">
		...
		<table class="table compare">
			...
			<tbody>
				...
				<?php
					...
					/*foreach ($apartments as $item) {
						$item->references = $item->getFullInformation($item->id, $item->type);
						...
						foreach($item->references as $ref) {
							if($ref['title']){
								...
								echo '<td>';
								echo '<strong>'.CHtml::encode($ref['title']).':</strong>';
								echo '</td>';
